...
(() => {
  const readLocal = (key, fallback = null) => {
    try {
      ...
      console.warn(`Phase 2 storage read failed: ${key}`, error);
      return fallback;
    }
  };
  ...
  const writeLocal = (key, value) => {
    try {
      ...
      console.warn(`Phase 2 storage write failed: ${key}`, error);
    }
  };

  const phase2WhatsAppBtn = document.querySelector("#phase2WhatsAppBtn");
  const phase2CallBtn = document.querySelector("#phase2CallBtn");
  const phase2EmailBtn = document.querySelector("#phase2EmailBtn");
  const phase2FaqBtn = document.querySelector("#phase2FaqBtn");

  if (phase2WhatsAppBtn) {
    phase2WhatsAppBtn.addEventListener("click", () => {
      const profile = readLocal("insuranceHubCustomerProfile", null);
      const quote = readLocal("insuranceHubDemoQuote", null);
      const message =
`Hello Insurance Hub Support,
...
Quote ID: ${quote?.quoteId || "Not available"}
...
Please assist me with the next step.`;
  ...
  if (phase2CallBtn) {
    phase2CallBtn.addEventListener("click", () => {
      const supportNumber = "";
      if (!supportNumber) {
        showToast("Support phone number is not configured yet.");
        return;
      }
  ...
  if (phase2EmailBtn) {
    phase2EmailBtn.addEventListener("click", () => {
      const supportEmail = "";
      if (!supportEmail) {
        showToast("Support email is not configured yet.");
        return;
      }
      window.location.href =
        `mailto:${supportEmail}?subject=${encodeURIComponent("Insurance Hub Support Request")}`;
    });
  }

  if (phase2FaqBtn) {
    phase2FaqBtn.addEventListener("click", () => {
  ...
  const supportForm = document.querySelector("#phase2SupportForm");
  const supportTicketResult = document.querySelector("#supportTicketResult");
  ...
  if (supportForm) {
    supportForm.addEventListener("submit", event => {
      ...
      const mobile = document.querySelector("#supportMobile")?.value.trim() || "";
      const type = document.querySelector("#supportType")?.value || "";
      const priority = document.querySelector("#supportPriority")?.value || "Normal";
      const message = document.querySelector("#supportMessage")?.value.trim() || "";

      if (!name || !mobile || !type || !message) {
        showToast("Please complete all required support fields.");
        return;
      }

      const ticket = {
        ticketId:`IH-SUP-${Date.now().toString().slice(-8)}`,
        name,
        mobile,
        type,
        priority,
      ...
      const tickets = readLocal("insuranceHubSupportTickets", []);
      tickets.unshift(ticket);
      writeLocal("insuranceHubSupportTickets", tickets);

      if (supportTicketResult) {
        supportTicketResult.hidden = false;
        supportTicketResult.innerHTML =
          `<strong>Support ticket created.</strong><br>
           Reference: ${escapeHTML(ticket.ticketId)}<br>
  ...
  const notifications = [
    {
      type:"QUOTE",
      title:"Quote update",
      text:"Your latest demo quote can be reviewed from My Quotes.",
      time:"Demo"
    },
    {
      type:"PAYMENT",
      title:"Payment update",
      text:"Demo payment status is available in My Payments.",
      time:"Demo"
    },
    {
      type:"POLICY",
      title:"Policy update",
      text:"Policy records generated by the demo flow appear in My Policies.",
      time:"Demo"
    },
    {
      type:"RENEWAL",
      title:"Renewal reminder",
      text:"Review your policy expiry date before renewal.",
      time:"Reminder"
    },
    {
      type:"CLAIM",
      title:"Claim update",
      text:"Track claim references and timeline status from Claims.",
      time:"Demo"
    }
  ];
  ...
  if (notificationList) {
    notificationList.innerHTML = notifications.map(item => `
      <article class="phase2-notification-card">
        <div class="phase2-notification-meta">
  ...
  const knowledgeData = {
    "insurance-guide": {
      ...
      text:"Insurance generally transfers specified financial risks to an insurer in exchange for a premium. Review coverage, exclusions, deductibles, limits, policy period and claim conditions before purchase."
    },
    "buying-guide": {
      title:"Buying Guide",
      text:"Compare the coverage you need, policy limits, exclusions, deductibles, waiting periods where applicable, insurer servicing process and the documents required."
    },
    "claim-guide": {
      title:"Claim Guide",
      text:"A claim normally involves notifying the insurer, providing the requested documents, verification and a decision under the applicable policy terms. Exact procedures vary by product and insurer."
    },
    "renewal-guide": {
      title:"Renewal Guide",
      text:"Check the policy expiry date, continuity of coverage, updated personal or vehicle information, premium changes and any revised terms before renewal."
    },
    "faq": {
      title:"Insurance FAQs",
      text:"Common questions include what a premium covers, how exclusions work, what documents may be needed for a claim and how renewal dates affect continuity."
    },
    "glossary": {
      title:"Insurance Glossary",
      text:"Premium means the amount paid for insurance. Sum insured or coverage limit refers to the applicable maximum coverage under a policy. Deductible is an amount the insured may have to bear under specified terms. Exclusion is a condition or event not covered by the policy."
    }
  };

  const knowledgeDetail = document.querySelector("#knowledgeDetail");

  document.querySelectorAll("[data-knowledge]").forEach(card => {
    card.querySelector("button")?.addEventListener("click", () => {
      const key = card.dataset.knowledge;
      const item = knowledgeData[key];
      if (!item || !knowledgeDetail) return;

      knowledgeDetail.hidden = false;
      knowledgeDetail.innerHTML =
        `<strong>${escapeHTML(item.title)}</strong><br>${escapeHTML(item.text)}`;

      knowledgeDetail.scrollIntoView({
        behavior:"smooth",
        block:"nearest"
      });
    });
  ...
  const calcConfig = {
    health: {
      title:"Health Insurance Estimate",
      description:"Educational estimate based on family size, age and monthly budget.",
      labels:["Family Members","Average Age","Monthly Budget (₹)"]
    },
    life: {
      title:"Life Cover Estimate",
      description:"Educational coverage estimate using income, age and monthly budget.",
      labels:["Annual Income (₹)","Age","Monthly Budget (₹)"]
    },
    motor: {
      title:"Motor Insurance Estimate",
      description:"Educational premium range based on vehicle value, age and budget.",
      labels:["Vehicle Value (₹)","Vehicle Age","Monthly Budget (₹)"]
    },
    travel: {
      title:"Travel Insurance Estimate",
      description:"Educational estimate using travellers, trip days and budget.",
      labels:["Travellers","Trip Days","Budget (₹)"]
    },
    home: {
      title:"Home Insurance Estimate",
      description:"Educational estimate using property value, contents value and budget.",
      labels:["Property Value (₹)","Contents Value (₹)","Budget (₹)"]
    },
    accident: {
      title:"Personal Accident Estimate",
      description:"Educational coverage estimate using income, age and budget.",
      labels:["Annual Income (₹)","Age","Monthly Budget (₹)"]
    }
  };

  let activeCalculator = "health";

  const updateCalculatorUI = () => {
    const config = calcConfig[activeCalculator];
    if (!config) return;

    const title = document.querySelector("#calculatorTitle");
    const description = document.querySelector("#calculatorDescription");
    const labels = [
      document.querySelector("#calcFieldOneLabel"),
      document.querySelector("#calcFieldTwoLabel"),
      document.querySelector("#calcFieldThreeLabel")
    ];
    ...
    labels.forEach((el, index) => {
      if (el) el.textContent = config.labels[index];
    });

    document.querySelectorAll(".phase2-calc-tab").forEach(tab => {
  ...
  document.querySelectorAll(".phase2-calc-tab").forEach(tab => {
    tab.addEventListener("click", () => {
      activeCalculator = tab.dataset.calculator || "health";
      updateCalculatorUI();
      const result = document.querySelector("#calculatorResult");
      if (result) result.textContent = "Enter your details and select Calculate Estimate.";
    });
  });

  const calculateInsuranceBtn = document.querySelector("#calculateInsuranceBtn");

  if (calculateInsuranceBtn) {
    calculateInsuranceBtn.addEventListener("click", () => {
      const a = Number(document.querySelector("#calcFieldOne")?.value || 0);
      const b = Number(document.querySelector("#calcFieldTwo")?.value || 0);
      const c = Number(document.querySelector("#calcFieldThree")?.value || 0);
      const result = document.querySelector("#calculatorResult");

      if (!result || a <= 0 || b <= 0 || c < 0) {
        showToast("Please enter valid calculator values.");
        return;
      }

      let estimate = 0;

      if (activeCalculator === "health") {
        estimate = Math.max(a * 300000, b * 5000);
      } else if (activeCalculator === "life") {
        estimate = Math.max(a * 10, c * 12);
      } else if (activeCalculator === "motor") {
        estimate = Math.max(a * 0.015, c * 12);
      } else if (activeCalculator === "travel") {
        estimate = Math.max(a * b * 500, c);
      } else if (activeCalculator === "home") {
        estimate = Math.max((a + b) * 0.001, c);
      } else if (activeCalculator === "accident") {
        estimate = Math.max(a * 5, c * 12);
      }

      result.innerHTML =
        `<strong>Educational estimate: ₹${Math.round(estimate).toLocaleString("en-IN")}</strong><br>
         This is a demo calculation only. Actual insurance pricing and coverage depend on the product, insurer, underwriting and policy terms.`;

      writeLocal("insuranceHubLastCalculator", {
...
(() => {
  const readLocal = (key, fallback = null) => {
    try {
      ...
      console.warn(`Phase 3 storage read failed: ${key}`, error);
      return fallback;
    }
  };

  const safeLength = key => {
  ...
  const adminViews = {
    ...
    customers:{title:"Customers", storage:"insuranceHubCustomerProfile", type:"single"},
    leads:{title:"Leads", storage:"insuranceHubLeads", type:"array"},
    products:{title:"Products", static:true},
    plans:{title:"Plans", storage:"insuranceHubDemoQuote", type:"single"},
    insurers:{title:"Insurers", static:true},
    quotes:{title:"Quotes", storage:"insuranceHubDemoQuote", type:"single"},
    applications:{title:"Applications", storage:"insuranceHubProposal", type:"single"},
    policies:{title:"Policies", storage:"insuranceHubDemoPolicies", type:"array"},
    payments:{title:"Payments", storage:"insuranceHubDemoPayment", type:"single"},
    claims:{title:"Claims", storage:"insuranceHubClaims", type:"array"},
    renewals:{title:"Renewals", storage:"insuranceHubRenewals", type:"array"},
    documents:{title:"Documents", storage:"insuranceHubProposal", type:"single"},
    whatsapp:{title:"WhatsApp Leads", storage:"insuranceHubLeads", type:"array"},
    notifications:{title:"Notifications", static:true},
    support:{title:"Support Tickets", storage:"insuranceHubSupportTickets", type:"array"},
    reports:{title:"Reports", static:true},
    settings:{title:"Settings", static:true}
  };

  const staticRecords = {
    products:[
      ["Motor Insurance","Active","Demo catalogue"],
      ["Health Insurance","Active","Demo catalogue"],
      ["Life Insurance","Active","Demo catalogue"],
      ["Travel Insurance","Active","Demo catalogue"],
      ["Home Insurance","Active","Demo catalogue"],
      ["Personal Accident","Active","Demo catalogue"]
    ],
    insurers:[
      ["Demo Insurer A","Motor","Demo"],
      ["Demo Insurer B","Motor","Demo"],
      ["Demo Insurer C","Motor","Demo"]
    ],
    notifications:[
      ["QUOTE","Quote update","Demo"],
      ["PAYMENT","Payment update","Demo"],
      ["POLICY","Policy update","Demo"],
      ["RENEWAL","Renewal reminder","Demo"],
      ["CLAIM","Claim update","Demo"]
    ],
    reports:[
      ["Customer Records","Local demo records","Available"],
      ["Quote Records","Local demo records","Available"],
      ["Payment Records","Local demo records","Available"],
      ["Claim Records","Local demo records","Available"],
      ["Support Records","Local demo records","Available"]
    ],
    settings:[
      ["Environment","Frontend Demo","Active"],
      ["Authentication","Demo UI","Not connected"],
      ["Payment","Demo Gateway","Not connected"],
      ["WhatsApp","Pending","Not configured"],
      ["Email","Pending","Not configured"]
    ]
  };

  const adminViewTitle = document.querySelector("#adminViewTitle");
  const adminOverview = document.querySelector("#adminOverview");
  const adminTableView = document.querySelector("#adminTableView");
  const adminSecurityView = document.querySelector("#adminSecurityView");
  const adminTableHead = document.querySelector("#adminTableHead");
  const adminTableBody = document.querySelector("#adminTableBody");
  const adminTableHeading = document.querySelector("#adminTableHeading");
  const adminTableKicker = document.querySelector("#adminTableKicker");
  ...
  const recordFromSingle = value => {
    if (!value || typeof value !== "object") return [];
    return [value];
  };

  const normalizeRecords = (key, type) => {
    if (type === "single") return recordFromSingle(readLocal(key, null));
    const value = readLocal(key, []);
    return Array.isArray(value) ? value : [];
  };

  const objectToRows = records => {
    if (!records.length) return {headers:[], rows:[]};

    const preferred = [
      "quoteId","applicationId","policyId","paymentId",
      "ticketId","claimId","renewalId","name","mobile",
      "email","planName","status","total","createdAt"
    ];

    const keys = [];
    preferred.forEach(key => {
      if (records.some(item => item && Object.prototype.hasOwnProperty.call(item,key))) {
        keys.push(key);
      }
    });
  ...
  const renderTable = (viewKey) => {
    const config = adminViews[viewKey];
    if (!config || config.static) {
      const data = staticRecords[viewKey] || [];
      adminTableKicker.textContent = "ADMIN DATA";
      adminTableHeading.textContent = config?.title || "Records";
      adminRecordCount.textContent = `${data.length} records`;

      adminTableHead.innerHTML = "<tr><th>Name</th><th>Category</th><th>Status</th></tr>";
    ...
    const records = normalizeRecords(config.storage, config.type);
    const {headers, rows} = objectToRows(records);

    adminTableKicker.textContent = "LOCAL DEMO RECORDS";
    adminTableHeading.textContent = config.title;
    adminRecordCount.textContent = `${records.length} records`;

    if (!headers.length) {
      adminTableHead.innerHTML = "";
      adminTableBody.innerHTML =
        `<tr><td>No records available in the current demo session.</td></tr>`;
      return;
    }
    ...
    adminTableBody.innerHTML = rows.map(row =>
  ...
  const refreshDashboard = () => {
    const profile = readLocal("insuranceHubCustomerProfile", null);
    const policies = readLocal("insuranceHubDemoPolicies", []);
    const payment = readLocal("insuranceHubDemoPayment", null);
    const claims = readLocal("insuranceHubClaims", []);
    const support = readLocal("insuranceHubSupportTickets", []);
    ...
    set("#adminCustomersCount", profile ? 1 : 0);
    set("#adminQuotesCount", safeLength("insuranceHubDemoQuote"));
    set("#adminPoliciesCount", Array.isArray(policies) ? policies.length : 0);
    set("#adminPaymentsCount", payment ? 1 : 0);
    set("#adminClaimsCount", Array.isArray(claims) ? claims.length : 0);
    set("#adminSupportCount", Array.isArray(support) ? support.length : 0);

    const activities = [];

    if (profile) activities.push(["Customer profile", "Customer profile is available"]);
    if (readLocal("insuranceHubDemoQuote", null)) activities.push(["Quote", "Demo quote available"]);
    if (payment) activities.push(["Payment", "Demo payment record available"]);
    if (Array.isArray(policies) && policies.length) activities.push(["Policy", "Policy record available"]);
    if (Array.isArray(claims) && claims.length) activities.push(["Claim", "Claim record available"]);
    if (Array.isArray(support) && support.length) activities.push(["Support", "Support ticket available"]);

    if (!activities.length) {
      adminRecentActivity.innerHTML =
        `<p class="muted">No activity in the current demo session.</p>`;
      return;
    }

    adminRecentActivity.innerHTML = activities.map(item => `
      <div class="phase3-audit-item">
  ...
  document.querySelectorAll(".phase3-admin-nav").forEach(button => {
    button.addEventListener("click", () => {
      const key = button.dataset.adminView || "overview";

      document.querySelectorAll(".phase3-admin-nav")
      ...
      if (adminViewTitle) {
        adminViewTitle.textContent =
          key === "security"
            ? "Security Center"
            : (adminViews[key]?.title || "Dashboard Overview");
      }

      if (adminOverview) adminOverview.hidden = key !== "overview";
      if (adminTableView) adminTableView.hidden = key === "overview" || key === "security";
      if (adminSecurityView) adminSecurityView.hidden = key !== "security";
  ...
  if (adminLogoutBtn) {
    adminLogoutBtn.addEventListener("click", () => {
      const status = document.querySelector("#adminSessionStatus");
      if (status) status.textContent = "DEMO LOGGED OUT";
      showToast("Demo admin session ended.");
    });
  }

  const renderAuditLog = () => {
    const target = document.querySelector("#securityAuditLog");
    if (!target) return;

    const logs = readLocal("insuranceHubAuditLogs", []);

    if (!logs.length) {
      target.innerHTML =
        `<div class="phase3-audit-item">
          <strong>No security events recorded</strong>
          <span>Audit records will appear when demo security actions are performed.</span>
        </div>`;
      return;
    }
    ...
    target.innerHTML = logs.map(log => `
      <div class="phase3-audit-item">
        <strong>${escapeHTML(log.action || "Security event")}</strong>
  ...
  const clearDemoDataBtn = document.querySelector("#clearDemoDataBtn");

  if (clearDemoDataBtn) {
    clearDemoDataBtn.addEventListener("click", () => {
      const confirmed = window.confirm(
        "Clear Insurance Hub demo data from this browser?"
      );

      if (!confirmed) return;

      const keys = [
        "insuranceHubCustomerProfile",
        "insuranceHubDemoQuote",
        "insuranceHubDemoPayment",
        "insuranceHubDemoPolicies",
        "insuranceHubProposal",
        "insuranceHubClaims",
        "insuranceHubRenewals",
        "insuranceHubSupportTickets",
        "insuranceHubLeads",
        "insuranceHubLastCalculator"
      ];

      keys.forEach(key => localStorage.removeItem(key));

      addAuditLog("Demo data cleared by admin");
      refreshDashboard();
      renderAuditLog();
...
(() => {
  ...
  const readLocal = (key, fallback = null) => {
    try {
      ...
      console.warn(`Phase 4 storage read failed: ${key}`, error);
      return fallback;
    }
  };
  ...
  const update = () => {
    const profile = readLocal("insuranceHubCustomerProfile", null);
    const quote = readLocal("insuranceHubDemoQuote", null);
    const proposal = readLocal("insuranceHubProposal", null);
    const payment = readLocal("insuranceHubDemoPayment", null);
    const policies = toArray(readLocal("insuranceHubDemoPolicies", []));
    const claims = toArray(readLocal("insuranceHubClaims", []));
    const renewals = toArray(readLocal("insuranceHubRenewals", []));
    ...
    const customerNameEl = document.querySelector("#phase4CustomerName");
    const statusEl = document.querySelector("#phase4CustomerStatus");

    if (customerNameEl) customerNameEl.textContent = customerName;

    if (statusEl) {
      const hasActivity = Boolean(profile || quote || proposal || payment);
      statusEl.textContent = hasActivity
        ? "Demo customer activity detected"
        : "No active demo profile";
    }

    const quoteCount = quote ? 1 : 0;
    const applicationCount = proposal ? 1 : 0;
    const policyCount = policies.length;
    const paymentCount = payment ? 1 : 0;
    ...
    set("#phase4QuoteCount", quoteCount);
    set("#phase4ApplicationCount", applicationCount);
    set("#phase4PolicyCount", policyCount);
    set("#phase4PaymentCount", paymentCount);

    // Keep these references available for final QA/debugging.
    window.insuranceHubFinalState = {
      profile,
      quote,
      proposal,
      payment,
  ...
  document.addEventListener("insuranceHubPaymentSuccess", update);
  window.addEventListener("storage", update);

  const accountBtn = document.querySelector("#phase4GoAccount");
  const adminBtn = document.querySelector("#phase4GoAdmin");
  const supportBtn = document.querySelector("#phase4GoSupport");

  accountBtn?.addEventListener("click", () => {
    document.querySelector("#customer-account")?.scrollIntoView({
      behavior: "smooth",
      block: "start"
    });
  });
  ...
  supportBtn?.addEventListener("click", () => {
    document.querySelector("#customer-support")?.scrollIntoView({
      behavior: "smooth",
      block: "start"
    });
  });
